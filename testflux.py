...
filename1 = "/home/addotson/data_AWSC/raw/{0}_{1}_{2}_conex-{3}".format(year, $
filename2 = "/home/addotson/data_AWSC/raw/{0}_{1}_{2}_conex-{3}".format(year, $
...
for j in range(0,k):
    ...
    if y1[j+err1]==0 and y[j+err]==0 and ROcount>10 and NFcount>10:
        ...
        if gwrf[j+err]!=0:#vol through prefilters
            ...
            if intpf[j+err]<0.1 and wwpf[j+err]<0.1:
                pfp[j]=feed[j+err1]
                pfp1[j]=fltr1[j+err1]
                pfp2[j]=fltr2[j+err1]
                pfpcount=pfpcount+1
